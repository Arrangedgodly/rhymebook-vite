...
const Definitions = ({ definitions }: DefinitionsProps) => {
  ...
  return (
    <div className="flex-col justify-start items-center w-1/5 md:block hidden my-auto m-5 max-h-[70vh] overflow-y-auto text-center">
      {definitions && (
        <>
              ...
              <div key={index}>
                ...
                {meaning.definitions.map((definition: any, index: number) => (
                  <div key={index}>
                    ...
                    {definition.example && (
                      <p className="text-md italic m-2">
                        "{definition.example}"
                      </p>
                    )}
                  </div>
